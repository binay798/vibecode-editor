...
const WebContainerPreview: React.FC<WebContainerPreviewProps> = ({
  ...
  console.log(templateData, "templateData", loadingState);
  ...
  useEffect(() => {
    async function setupContainer() {
      ...
      try {
        ...
        // Create a promise to track when output streaming is complete
        const outputDone = installProcess.output.pipeTo(
          new WritableStream({
            write(data) {
              if (terminalRef.current?.writeToTerminal) {
        ...
        // Wait for BOTH the process to exit AND output to finish
        ...
        console.log(installExitCode, "exit codee");
        if (installExitCode !== 0) {
          throw new Error(
            `Failed to install dependencies. Exit code: ${installExitCode}`
          );
        }
        console.log("hello2", installExitCode);
